...
# --------------------------------------------------------------- Methodology
    ...
    st.markdown(
        ...
        "**Implausible or placeholder contract values are a data-quality issue, not a "
        "red flag.** They are classified into tiers, kept out of money totals where they "
        "are clearly junk, and listed in full on the **Data Quality** page."
    )
    ...
    st.markdown(
        ...
        "example BHEL), and a small number of awards carry impossibly large or "
        "placeholder values (for example a value ending in `12345678`). Missing values "
        "and high-confidence junk are excluded from value totals and rankings but kept "
        "for competition analysis, and every anomaly is listed on the **Data Quality** "
        "page. A value-disclosed percentage is shown so a reporting gap is never "
